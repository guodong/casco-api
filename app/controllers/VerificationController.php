    ...
    public function export()
    {
        ...
        $column_config = array(
            array(
                ...
                'Borders' => PHPExcel_Style_Border::BORDER_THICK,
                'Fill' => array(
        ...
        foreach ($column_config as $val) {
            $objPHPExcel->getActiveSheet()
                ->getColumnDimension($val['Col'])
                ->setWidth($val['Width']);
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($val['Col'] . $num, $val['text']);
            $objPHPExcel->getActiveSheet()
                ->getStyle($val['Col'] . $num)
                ->getFont()
                ->setBold($val['font']['Bold']);
            $objPHPExcel->getActiveSheet()
                ->getStyle($val['Col'] . $num)
                ->getFont()
                ->setName($val['font']['Name']);
            $objPHPExcel->getActiveSheet()
                ->getStyle($val['Col'] . $num)
                ->getFont()
                ->setSize($val['font']['size']);
            $objPHPExcel->getActiveSheet()
                ->getStyle($val['Col'] . $num)
                ->getBorders()
                ->getAllBorders()
                ->setBorderStyle($val['Borders']);
            $objPHPExcel->getActiveSheet()
                ->getStyle($val['Col'] . $num)
                ->getFill()
                ->setFillType($val['Fill']['Type']);
            $objPHPExcel->getActiveSheet()
                ->getStyle($val['Col'] . $num)
                ->getFill()
                ->getStartColor()
                ->setARGB($val['Fill']['Color']);
        }
        $num ++;
        foreach ($vefs as $v) {
            $i = 1;
            ...
            foreach ($column_config as $val) {
                $objPHPExcel->getActiveSheet()
                    ...
                    ->getAlignment()
                    ->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue($val['Col'] . $num, $data[$num1 ++]);
                $objPHPExcel->getActiveSheet()
                    ->getStyle($val['Col'] . $num)
                    ->getFont()
                    ->setName($val['font']['Name']);
